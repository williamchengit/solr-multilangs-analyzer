...
import com.worksap.labs.solr.helper.MultiLangIdentifierSettingHelper;
import com.worksap.labs.solr.schema.MultiLangField;
import com.worksap.labs.solr.setting.MultiLangFieldSetting;
import com.worksap.labs.solr.setting.MultiLangIdentifierSetting;
...
public abstract class MultiLangIdentifierUpdateProcessor extends UpdateRequestProcessor {
	...
	private void predefineLangToMultiLangField(SolrInputDocument doc, String fieldParam) {
		...
		if (null != inputField && null != inputField.getValues()) {
			for (Object value : inputField.getValues()) {
				...
				if (value instanceof String) {
					List<DetectedLanguage> detectedLanguages = detectLanguage((String) value);
					langSet = resolveLanguage(detectedLanguages, this.setting.getFallback());
				}
				...
				newValue = predefinedLangs + (String) newValue;
